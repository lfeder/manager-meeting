# Speaker notes

Edit freely. One `## <slide number>` heading per slide; every line under it
becomes a bullet in the notes panel. Press **N** while presenting to show or hide
them — the setting sticks between reloads.

Anything not under a `##` heading (like this) is ignored.

## 1 — Cover
The purpose of this talk: a quick recap of how things are going, and where we're heading.
The two biggest opportunities in front of us are embrace AI even more to make the farm even more personally rewarding and fullfill our mission / obligation to grow more food for Hawaii

## 2 — The WSJ page, then our answer
Take stock of what we've achieved. Step back from the fray and it's kind of crazy.
That's Larry Ellison's farm. Same state, same crop, half a billion dollars. They looked at the market and quit.
Then click for the answer.

## 3 — The greenhouse lettuce market
We literally compete against the wealthiest people in the world and we neat them. 
Punch that we are approaching industry best yields and provide the freshest, safest, and most affordable lettuce in Hawaii
## 4 — Food safety
We are likely the first farm in the world that is 100% PCR. For two years in a row we score 6 × 100%. Bruce says this is unprecedented. Big take away - How nervous are you about audits today? This is the power of embracing tech.


## 5 — People
We find people from around the world, pick the best, and bring them here. It's the American dream.

## 6 — The common denominator
Let the room answer it before you do.
Every one of those was done by this room. Nobody replaced, nobody hired over.

## 7 — Financial update

## 8 — Keep Experimenting
We tried growing longer. 15% more greenhouse time for 1.5% more pounds.
Did we need more time, or more labor? Keep trying

## 9 — Hard Challenge, the awkward lettuce work week
Tuesday and Friday the lettuce house is dark. What do we do with all this labor? 
Most go to Cuke packing, but for the same reason Lettuce is dark, Cukes is the slowest twodays of the week
Let get better schedule tracking. Let's get more creative with using the available crew: service, maint, etc. 

## 10 — Lettuce pack productivity
Prod dropped recently - it's a micro version of the above - fill all 10 hours. 

## 11 — Expansion I
Just like FS is no longer stressful - the point of 3.0 is to remove friction, allow everyone to elevate

## 12 — Paper → AI
2.0 is critical - we now have infinite flexibility - please dream up the craiziest thing you can think of

## 13 — Today = Friction
Read a few, don't read them all. The room will recognise their own week.
Busy is the four-letter word.

## 14 — Judgment call
Build the table one row at a time and let the room do the multiplication.
The work moves upstream: battle-test the reasoning, define what good means. Every wet day makes the next one better.

## 15 — What we will do
The glossary first — we did it with the Site Map
Give Claude everything, Julian will help to translation you -> Claude.
The intent behind standalone xls is great -> but going forward build it in Claude. 
This is an entry ticket to future elevation
Training materials came free.

## 16 — Future = Frictionless
Every hour we take out goes back to invention, relationships, and problems nobody has solved.
Like being back in school — learning something new every day.

## 17 — Expansion II
The farm gets bigger.

## 18 — Why we must
Our mission is to provide as much fresh, healthy food as possible to Hawaii. So, if we can expand, we must expand.

## 19 — Do the customers want it?
Winter shorting is now year-round shorting. 30% shorting in the summer!

## 20 — There is more where that came from
We have more opportunity than ever!

## 21 — Can we execute it?
Maria to sales full-time, with a backfill behind her.
Turn-key greenhouse construction company for the build.
Risk is lower with cucumbers than it was with lettuce

## 22 — Here is the plan
More greenhouses, new packhouse, two new houses.
This is why the last twenty-five minutes mattered. 

## 23 — Takeaway
Aim to not be busy. Swap judgement calls with pre-planning. Put everythign inside Aloha (you can be a technologist), Think Small / Think Big / What's the next impossible we're going to do
What's the goal again - reflect on food safety audits - let's massively embrace AI to elevate the joy of working at the farm for everyone
